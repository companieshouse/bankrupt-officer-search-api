...
if [[ -z "${MESOS_SLAVE_PID}" ]]; then
    ...
    source ~/.chs_env/bankrupt-officer-search-api/env

    PORT="${BANKRUPT_OFFICER_SEARCH_API_PORT:=18588}"
else
    PORT="$1"
    CONFIG_URL="$2"
    ENVIRONMENT="$3"
...
exec java ${JAVA_MEM_ARGS} -jar -Dserver.port="${PORT}" "${APP_DIR}/bankrupt-officer-search-api.jar"
